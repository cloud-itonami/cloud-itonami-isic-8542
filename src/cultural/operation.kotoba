(ns cultural.operation
  "OperationActor -- one cultural-education operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (StudioEdOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Instruction Integrity
  ...
  One graph run = one cultural-education operation (intake -> advise
  -> govern -> decide -> commit | hold | approval). No unbounded
  ...
  decision to a human operator (licensed educator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/finalize-certification` ALWAYS reaches this node when
  the governor is clean -- see `cultural.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [cultural.culturaladvisor :as culturaladvisor]
            [cultural.governor :as governor]
...
(defn build
  ...
  `cultural.store/Store`).
  opts:
    :advisor      -- a `cultural.culturaladvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (culturaladvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; StudioEdOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (culturaladvisor/-advise advisor store request)]
            {:proposal p :audit [(culturaladvisor/trace request p)]})))

      ;; Instruction Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
